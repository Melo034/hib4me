...
const Causes = () => {
  ...
  useEffect(() => {
    const fetchCauses = async () => {
      try {
        const response = await fetch("https://hib4me-server.onrender.com/api/donations/all"); // Update with your correct API endpoint
        if (!response.ok) {
              ...
              {causes.map((item, idx) => (
                ...
                >
                  <div className="w-full h-96 sm:w-1/2">
                    <img
                      src={`https://hib4me-server.onrender.com${item.image}`} 
